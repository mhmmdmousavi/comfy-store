    ...
    return(
        <div className="flex flex-col w-full">
            <div className="flex flex-row bg-blue-950  justify-end gap-4 pr-10 text-gray-300 text-sm py-1">
                <Link to={"/login"}>{isloggedin? "Hello dear user" : "Sign in / Guest"}</Link>
                <Link className={`${isloggedin? "hidden": "flex"}`} to={"/register"}>Create Account</Link>
                <button className={`${isloggedin?"flex": "hidden"}`} onClick={()=>{logOut()}}>Log out</button>
            </div>
            <div className="bg-sky-50 flex flex-row justify-between px-10 items-center">
                ...
                <ul className="flex flex-row gap-2 items-center text-sm">
                    ...
                    <li className="bg-sky-50 text-black px-2 py-1 rounded-md hover:bg-gray-200"><Link to={"/cart"}>Cart</Link></li>
                    <li className={`bg-sky-50 text-black px-2 py-1 rounded-md hover:bg-gray-200 ${isloggedin ? "flex" : "hidden"}`}><Link to={"/checkout"}>Checkout</Link></li>
                    <li className={`bg-sky-50 text-black px-2 py-1 rounded-md hover:bg-gray-200 ${isloggedin ? "flex" : "hidden"}`}><Link to={"/order"}>Order</Link></li>
                </ul>
